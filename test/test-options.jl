...
using TethysChlorisCore: find_bracket
...
@testset "find_root integration" begin
    @testset "Missing method" begin
        ...
        @test_throws ArgumentError find_root(x -> x^2 - 4, 1.0, strategy)
    end
    ...
    @testset "Failing to find root due to no sign change" begin
        f(x) = x^2 + 1  # No real roots
        bracket = (-1.0, 1.0)
        strategy = SimpleBrentStrategy(Float64)

        @test_throws Exception find_root(f, bracket, strategy)
    end

    @testset "Find new bracket when no sign change in bracket" begin
        f(x) = x^2 - 4
        bracket = (-3.0, 6.0)  # Does not contain a sign change
        strategy = SimpleBrentStrategy(Float64)

        @test isa(find_bracket(f, bracket, 1), Tuple)
        new_bracket = find_bracket(f, bracket, 1)
        @test_nowarn find_root(f, new_bracket, strategy)
    end
end
